...
err_vec_table = table_all(table_all.inc == 1, :); %% exclude failed responses
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
enc_x = err_vec_table.enc_x;
enc_y = err_vec_table.enc_y;
...
for x_coord_idx = 1:length(x_coord_list)
    for y_coord_idx = 1:length(y_coord_list)
        ...
        tag_window_target = ((x_coord-win_size/2)<=enc_x)&((x_coord+win_size/2)>=enc_x)&((y_coord-win_size/2)<=enc_y)&((y_coord+win_size/2)>=enc_y);
        
        curr_coin = [enc_x(tag_window_target),enc_y(tag_window_target)];

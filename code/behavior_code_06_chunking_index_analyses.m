...
correct_err(table_all.inc == 0) = NaN;
incorrect_err = table_all.err;
incorrect_err(table_all.inc == 1) = NaN;
...
for sbj_i = 1:num_of_sbj
    ...
    [proto_enc_dist(sbj_tag), proto_enc_idx(sbj_tag)] =  min(pdist2(curr_center,[table_all.enc_x(sbj_tag), table_all.enc_y(sbj_tag)]),[],1);
    ...
    proto_enc_tag(sbj_tag) =  proto_enc_dist(sbj_tag)<curr_size;
    proto_resp_tag(sbj_tag) =  proto_resp_dist(sbj_tag)<curr_size;

    chunking_idx(sbj_i) = (mean(proto_resp_tag(sbj_tag)) - mean(proto_enc_tag(sbj_tag)))/mean(proto_enc_tag(sbj_tag))*100;
    % chunking_idx(sbj_i) = (mean(proto_enc_dist(sbj_tag))/mean(proto_resp_dist(sbj_tag))) - 1;

end

save(fullfile(data_dir,'chunking_info.mat'), 'chunking_idx','proto_coin_tag','proto_enc_tag','proto_resp_tag')
...
for sbj_i = 1:num_of_sbj
    ...
    for data_i = 1:data_len
        ...
        if mean(proto_enc_tag(data_tag)) == 0
            chunking_idx_vars(sbj_i,data_i) = NaN;
            continue;
        end
        chunking_idx_vars(sbj_i,data_i) = (mean(proto_resp_tag(data_tag)) - mean(proto_enc_tag(data_tag)))/mean(proto_enc_tag(data_tag))*100;
        % chunking_idx_vars(sbj_i,data_i) = (mean(proto_enc_dist(data_tag))/mean(proto_resp_dist(data_tag))) - 1;

...
% code
sbj_coin_all = {}; sbj_resp_all = {}; sbj_diff_all = {};
parfor sbj_i = 1:length(sbj_list) 

    sbj_tag = use_data.sbj == sbj_list(sbj_i) %& (proto_coin_tag' == 0); %% if testing for off-node coins
    sbj_table = use_data(sbj_tag,:); 
    coin_x = sbj_table.coin_x;
    coin_y = sbj_table.coin_y;
    enc_x = sbj_table.enc_x;
    enc_y = sbj_table.enc_y;
    ...
    for x_coord_idx = 1:length(x_coord_list)
        ...
        for y_coord_idx = 1:length(y_coord_list)
            ...
            tag_window_enc = ((x_coord-win_size/2)<=enc_x)&((x_coord+win_size/2)>=enc_x)&((y_coord-win_size/2)<=enc_y)&((y_coord+win_size/2)>=enc_y);
            ...
            if ~(sum(tag_window_resp)) && ~(sum(tag_window_enc)) % if there is no response or encoding --> NaN
    ...
    num_diff_gau = num_resp_gau - num_coin_gau;
    ...
    num_diff_gau = (num_diff_gau - nanmean(num_diff_gau(:)))./std(num_diff_gau(:),'omitnan');
    

    sbj_coin_all{sbj_i} = num_coin_gau; %num_coin;  
    sbj_resp_all{sbj_i} = num_resp_gau; %num_resp; 
    sbj_diff_all{sbj_i} = num_diff_gau; %subtracted map

end

% % save data
% save([data_dir, '/sbj_pattern_all.mat'],'sbj_coin_all','sbj_resp_all','sbj_diff_all')
...
% Order of the participant may differ from the Supplementary Fig 1c
figure;
hold on

for sbj_i = 1:length(visu_data)
    subplot(num_fig_row, num_fig_col, sbj_i)
    hold on

    heatmap_data = visu_data{sbj_i}';
...
use_data = sbj_diff_all;% sbj_coin_all; % sbj_diff_all
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for row_i = 1:size(use_data{1},1)
    for col_i = 1:size(use_data{1},2)
        if sum(isnan(cellfun(@(x) x(row_i, col_i), use_data(:)))) >= (length(use_data)-2)

...
interval_name_list ={'0-25', '25-50', '50-75', '75-100'};

% code
err_coin_map = {};err_resp_map = {};
for sbj_i = 1:num_of_sbj
    indiv_err_thres = cellfun(@(x) prctile(use_data.err(use_data.sbj == sbj_i), x), thres_list);
    curr_data_sbj = {};
    for thres_i = 1:length(thres_list)-1

        target_err_thres = err_thres; %indiv_err_thres
        err_low_bnd = target_err_thres(thres_i);
        err_up_bnd = target_err_thres(thres_i+1);

        curr_table = use_data((use_data.sbj == sbj_i)&(use_data.err >= err_low_bnd)&(use_data.err <= err_up_bnd),:); 
...
num_fig_col = length(thres_list)-1;
num_fig_row = 1;
...
for err_i = 1:num_of_errs
    ...
    c.LineWidth = 1.25;
    clim([-4 4])
    set(gca,'LineWidth',1.5, 'FontWeight', 'bold','FontSize',14,'FontName','Arial');
    box on
...
ttest_data = loo_ps_fishz_all;
target_variable = 'err';
t = array2table(ttest_data);
t.Properties.VariableNames = {'meas1', 'meas2', 'meas3', 'meas4'};
Meas = table([1 2 3 4]','VariableNames',{target_variable});
...
% t-test results
[h, p,~,stats] = ttest(loo_ps_fishz_all);
sprintf('%0.3f  ',p)

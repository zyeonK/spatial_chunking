...
map_name  = 'sbj_pattern_all.mat'; % 'sbj_coin_all','sbj_resp_all', 'sbj_diff_all'
...
for scale_i = 1:length(scale_list)
    ...
    for size_i = 1:length(sig_list)
        ...
        mdl_i = mdl_i + 1;
        rho_list = cellfun(@(x) corr(x(:), rand_data(:),'type', corr_type,'rows','pairwise'), use_data);
        ...
        % Pearson R
        rho_list = cellfun(@(x) corr(x(:), trunc_gaussian_all(:),'type', corr_type,'rows','pairwise'), use_data);
...
% save data
% save(['multi_node_model_fit.mat'], 'rho_list_0_all', 'rho_list_4_all','rho_list_5_all','rho_list_9_all','rho_list_16_all')
...
%% Visualization
% Histogram showing number of participants whose responses most highly correlated with each chunking model

avg_data = nanmean(max_rho_all,2);
std_data = std(max_rho_all,[],2);
result_strings = compose("%.3f \xB1 %.3f", avg_data, std_data);
disp(result_strings);
...
fprintf('one-sample t-test against zero\n')
p=[];
[h,p(1),~,stats] = ttest(max_rho_all(1,:));
d = stats.tstat / sqrt(stats.df+1);
fprintf('0-node: p = %.3f, t(%d) = %.3f, Cohen''s d: %.3f\n', p(1), stats.df, stats.tstat, d)
[h,p(2),~,stats] = ttest(max_rho_all(2,:));
d = stats.tstat / sqrt(stats.df+1);
fprintf('4-node: p = %.3f, t(%d) = %.3f, Cohen''s d: %.3f\n', p(2), stats.df, stats.tstat, d)
[h,p(3),~,stats] = ttest(max_rho_all(3,:));
d = stats.tstat / sqrt(stats.df+1);
fprintf('5-node: p = %.3f, t(%d) = %.3f, Cohen''s d: %.3f\n', p(3), stats.df, stats.tstat, d)
[h,p(4),~,stats] = ttest(max_rho_all(4,:));
d = stats.tstat / sqrt(stats.df+1);
fprintf('9-node: p = %.3f, t(%d) = %.3f, Cohen''s d: %.3f\n', p(4), stats.df, stats.tstat, d)
[h,p(5),~,stats] = ttest(max_rho_all(5,:));
d = stats.tstat / sqrt(stats.df+1);
fprintf('16-node: p = %.3f, t(%d) = %.3f, Cohen''s d: %.3f\n', p(5), stats.df, stats.tstat, d)
[a,b,c,d] = fdr_bh(p)

% multi-node-modeling results (comparison with 0-node model)
fprintf('\n\npaired t-test with 0-node model\n')
p=[];
[h,p(1),~,stats] = ttest(max_rho_all(2,:),max_rho_all(1,:));
d = stats.tstat / sqrt(stats.df+1);
fprintf('4-node: p = %.3f, t(%d) = %.3f, Cohen''s d: %.3f\n', p(1), stats.df, stats.tstat, d)
[h,p(2),~,stats] = ttest(max_rho_all(3,:),max_rho_all(1,:));
d = stats.tstat / sqrt(stats.df+1);
fprintf('5-node: p = %.3f, t(%d) = %.3f, Cohen''s d: %.3f\n', p(2), stats.df, stats.tstat, d)
[h,p(3),~,stats] = ttest(max_rho_all(4,:),max_rho_all(1,:));
d = stats.tstat / sqrt(stats.df+1);
fprintf('9-node: p = %.3f, t(%d) = %.3f, Cohen''s d: %.3f\n', p(3), stats.df, stats.tstat, d)
[h,p(4),~,stats] = ttest(max_rho_all(5,:),max_rho_all(1,:));
d = stats.tstat / sqrt(stats.df+1);
fprintf('16-node: p = %.3f, t(%d) = %.3f, Cohen''s d: %.3f\n', p(4), stats.df, stats.tstat, d)
[a,b,c,d] = fdr_bh(p)

% multi-node modeling results (comparison with 9-node model)
fprintf('\n\npaired t-test with 9-node model\n')
p=[];
[h,p(1),~,stats] = ttest(max_rho_all(1,:),max_rho_all(4,:));
d = stats.tstat / sqrt(stats.df+1);
fprintf('0-node: p = %.3f, t(%d) = %.3f, Cohen''s d: %.3f\n', p(1), stats.df, stats.tstat, d)
[h,p(2),~,stats] = ttest(max_rho_all(2,:),max_rho_all(4,:));
d = stats.tstat / sqrt(stats.df+1);
fprintf('4-node: p = %.3f, t(%d) = %.3f, Cohen''s d: %.3f\n', p(2), stats.df, stats.tstat, d)
[h,p(3),~,stats] = ttest(max_rho_all(3,:),max_rho_all(4,:));
d = stats.tstat / sqrt(stats.df+1);
fprintf('5-node: p = %.3f, t(%d) = %.3f, Cohen''s d: %.3f\n', p(3), stats.df, stats.tstat, d)
[h,p(4),~,stats] = ttest(max_rho_all(5,:),max_rho_all(4,:));
d = stats.tstat / sqrt(stats.df+1);
fprintf('16-node: p = %.3f, t(%d) = %.3f, Cohen''s d: %.3f\n', p(4), stats.df, stats.tstat, d)
[a,b,c,d] = fdr_bh(p)
